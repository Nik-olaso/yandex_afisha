...
Часть настроек проекта берется из переменных окружения. Создайте файл `.env` в корневой директории и заполните его следующими данными:
  ...
  ALLOWED_HOSTS — Список строк, представляющих имена хостов/доменов, которые может обслуживать этот сайт Django.Записываетсяв виде адрес1,адрес2,адрес3. (Необязательная настройка)
  ```
...
Для добавления новых мест используется скрипт load_place.py
